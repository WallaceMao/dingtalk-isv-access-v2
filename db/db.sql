...
# 在isv_corp_callback_queue中增加时间戳
ALTER TABLE `isv_corp_callback_queue`
  ADD COLUMN `timestamp`  bigint(20) NOT NULL DEFAULT 0 COMMENT '回调事件的时间戳，用来保证顺序',
  ADD COLUMN `status`  bigint(20) NOT NULL DEFAULT 0 COMMENT '回调事件的状态',
  ADD INDEX `u_c_c_queue_status`(`corp_id`, `status`) USING BTREE;
# 增加索引
ALTER TABLE `isv_corp_callback_queue`
  ADD INDEX `index_corp_callback_status_timestamp`(`status`, `timestamp`) USING BTREE;
# auth应用中的全局锁
CREATE TABLE `isv_auth_global_lock`  (
