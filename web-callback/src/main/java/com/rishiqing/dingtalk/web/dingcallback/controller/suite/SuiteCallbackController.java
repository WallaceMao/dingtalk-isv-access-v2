...
public class SuiteCallbackController {
    ...
    private void isvCallbackEvent(String callbackMsg) {
        ...
        if(SuitePushType.SUITE_TICKET.getKey().equals(eventType)){
            ...
            activateSuite(corpSuiteAuth.getTempAuthCode(), corpSuiteAuth.getCorpId());
     ...
     * @return
     */
    private void activateSuite(String tmpCode, String corpId) {
        SuiteVO suiteVO = suiteManager.getSuite();
        SuiteTokenVO suiteTokenVO = suiteManager.getSuiteToken();

        CorpSuiteAuthVO suiteAuthVO = callbackRequestHelper.getPermanentCode(
                suiteVO.getSuiteKey(), corpId, tmpCode, suiteTokenVO.getSuiteToken());
        if (suiteAuthVO != null) {
            callbackRequestHelper.activeSuite(suiteVO.getSuiteKey(), corpId, suiteTokenVO.getSuiteToken());
            suiteCallbackBizService.receiveTmpAuthCode(suiteAuthVO);
        }
    }
}
