...
import com.rishiqing.dingtalk.web.util.http.BizRequestHelper;
...
public class SuiteCallbackController {
    ...
    @Autowired
    private BizRequestHelper bizRequestHelper;
    @Autowired
    ...
    private void isvCallbackEvent(String callbackMsg) {
        ...
        if(SuitePushType.SUITE_TICKET.getKey().equals(eventType)){
            ...
            //  接收到临时授权码之后，执行开通操作
            CorpSuiteAuthVO corpSuiteAuth = SuiteCallbackConverter.json2CorpSuiteAuth(jsonMessage);
            corpSuiteAuth = activateSuite(
                    corpSuiteAuth.getTempAuthCode(), corpSuiteAuth.getCorpId());
            suiteCallbackBizService.receiveTmpAuthCode(corpSuiteAuth);
     ...
     * @return
     */
    private CorpSuiteAuthVO activateSuite(String tmpCode, String corpId) {
        SuiteVO suiteVO = suiteManager.getSuite();
        SuiteTokenVO suiteTokenVO = suiteManager.getSuiteToken();

        CorpSuiteAuthVO suiteAuthVO = bizRequestHelper.getPermanentCode(
                suiteVO.getSuiteKey(), tmpCode, suiteTokenVO.getSuiteToken());
        bizRequestHelper.activeSuite(suiteVO.getSuiteKey(), corpId, suiteTokenVO.getSuiteToken());
        return suiteAuthVO;
    }
}
